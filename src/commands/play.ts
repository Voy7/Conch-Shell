...
export async function run(input: CommandInput) {
  ...
  // If is a YouTube playlist URL
    ...
    if (!musicPlayer.isSilentMode) {
      input.reply(
        `:mag_right: \`Fetching playlist data for "${input.args[0]}", please wait...\``
      );
    }
    try {
      ...
      if (!playlist)
        return input.reply(
          `:x: \`Error: Could not get playlist info for "${input.args[0]}"\``
        );

      const videos: Video[] = await playlist.getVideos();
      if (videos.length === 0)
        return input.reply(
          `:x: \`Error: Playlist "${playlist.title}" contains no videos!\``
        );

      const ytdlInfo = await Promise.all(
        videos.map((video) => ytdl.getInfo(video.url))
      );
      ...
      videos.forEach((video, index) => {
        const playable = new Playable(
          ...
          video.url,
          { videoInfo: video, ytdlInfo: ytdlInfo[index] },
          true
        );
        musicPlayer.addPlayable(playable);
        if (musicPlayer.isSilentMode) input.deleteMessage();
      ...
      handleYouTubeError(error, input);
    }
  }
  ...
  // If is a YouTube URL
    ...
    const isShorts = input.args[0]?.match(
      /^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.?be)\/shorts\/.+$/
    );
    const parsedURL = isShorts
      ? input.args[0].split("?")[0].replace("/shorts/", "/watch?v=")
      : input.args[0];
    ...
    try {
      ...
      if (!videoInfo || !ytdlInfo) {
        return input.reply(
          `:x: \`Error: Could not get video info for "${input.args[0]}"\``
        );
      }
      const playable = new Playable(
        input,
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        const playable = new Playable(
          ...
          PlayableType.YouTube,
          results[0].url,
          { videoInfo: results[0], ytdlInfo }
        );
        musicPlayer.addPlayable(playable);
        if (musicPlayer.isSilentMode) input.deleteMessage();
      } else
        input.reply(`:x: \`No search results found for: "${input.args[0]}"\``);
    } catch (error: any) {
      handleYouTubeError(error, input);
    }
  }
...
async function getFileInfo(url: string): Promise<FileInfo | null> {
  return new Promise((resolve, reject) => {
    ffmpeg.ffprobe(url, (error, metadata) => {
      ...
      const name = metadata?.format?.filename
        ? metadata.format.filename.split("/").pop()?.split("?")[0]
        : null;
...
function handleYouTubeError(error: any, input: CommandInput) {
  ...
  if (
    ...
    input.reply(
      `:x: \`Video is age-restricted or has a similar content restriction. (${statusCode})\``
    );
  }
