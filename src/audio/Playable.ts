...
export default class Playable {
  ...
  get title(): string {
    ...
    if (this.extraInfo?.videoInfo?.title)
      title = this.extraInfo.videoInfo.title;
    else if (this.extraInfo?.fileInfo?.name)
      title = this.extraInfo.fileInfo.name;
  ...
  get currentDuration(): number {
    return Math.floor(
      this.resource.playbackDuration / 1000 + this.startedTimeSeconds
    );
  }

  // Get total/max duration of the Playable in seconds, rounded down
  get totalDuration(): number {
    if (this.extraInfo?.ytdlInfo?.videoDetails.lengthSeconds) {
      // is YouTube video
      return parseInt(this.extraInfo.ytdlInfo.videoDetails.lengthSeconds);
    }
    if (this.extraInfo?.fileInfo?.durationSeconds) {
      // is File
  ...
  get thumbnail(): string {
    if (this.extraInfo?.videoInfo?.thumbnails.high.url) {
      // is YouTube video
  ...
  get channel(): string {
    if (this.extraInfo?.videoInfo?.channel.title) {
      // is YouTube video
